...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_redirect_if_logged_in_but_not_correct_permission(self):
        login = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.get(reverse('renew-book-librarian', kwargs={'pk': self.test_bookinstance1.pk}))
        # print('!!!!!!!!!!', reverse('renew-book-librarian', kwargs={'pk': self.test_bookinstance1.pk}))
        self.assertEqual(response.status_code, 302)
...
class AuthorCreateViewTest(TestCase):
    ...
    def test_refuse_if_logged_in_but_not_correct_permission(self):
        login = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.get(reverse('author_create'))
    ...
    def test_form_renewal_date_initially_has_death_date(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 200)
        # date = datetime.datetime.strptime("05/01/2018", "%d/%m/%Y").date() # datetime.date('05/01/2018')
        self.assertEqual(response.context['form'].initial['date_of_death'], '05/01/2018')

    def test_redirect_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        # response = self.client.get(reverse('author_create'))
        response = self.client.post(reverse('author_create'), {'first_name':'John', 'last_name': 'Doe',
                                                    'date_of_birth': '01/01/2018', 'date_of_death': '05/01/2018'})

        # response = self.client.get(reverse('my-borrowed'))
        self.assertEqual(response.status_code, 302)
        self.assertTrue(response.url.startswith(reverse('author-detail', kwargs={'pk': 0})[:-1]))
